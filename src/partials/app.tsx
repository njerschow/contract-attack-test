...
const App = () => {
  ...
  }, [contractAddress, account, isActive])
        ...
        contract
          ...
          .then(txRcpt => {
            ...
            toast.success(
              <>
                Successfully pwned:{" "}
                <a target="_blank" rel="noreferrer" style={{color: "#0066FF"}} href={`https://testnet.snowtrace.io/tx/${txRcpt.transactionHash}`}>View in explorer</a>
              </>,
            )
          })
    ...
    <div style={{ marginBottom: 32, background: "transparent" }}>
      ...
      >
        ...
        >
          ...
          <button className={balance.gt(0) ? "action-button" : "action-button smaller disabled"} onClick={stealFunds}>
            {!isActive ? "Connect your wallet" : balance.gt(0) ? "Steal funds" : "Contract is already drained!"}
          </button>
        </div>
        <button
          style={{ marginTop: 32 }}
          className={pwnerBalance.gt(0) ? "action-button smaller squircle" : "action-button smaller disabled"}
